...
export default function handler(req: any, res: any) {
  if (MARKETPLACE_CONSUMER_MODE) {
    return proxyGangLedgerJson(req, res, '/api/integrations/marketplace/orders/admin', {
      allowMethods: ['GET'],
      notSupportedMessage: 'Marketplace admin orders are managed by Gang Ledger.',
    })
  }

  try {
    const url = String(req.url || '')
    ;(req as any).url = url.replace(/^\/api\/admin\/orders/, '/admin/orders')
  } catch {}
